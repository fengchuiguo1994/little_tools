...
ftmp = writeFile("{0}.tmp.map.txt".format(sys.argv[3]))
resFrag = {}
for line in fanchor:
    ...
    if exstart < 0:
        exstart = 0
    exend = end + 75
    ftmp.write("{0}\t{1}\t{2}\t{0}\t{3}\t{4}\n".format(chrom,start,end,exstart,exend))
    regionTree([chrom,exstart,exend],resFrag)
fanchor.close()
ftmp.close()
...
for k1 in loopdict.keys():
    for k2 in loopdict[k1].keys():
        floop.write("{0}\t{1}\t{2}\t{3}".format(k1,k2,loopdict[k1][k2],covdict[k1],covdict[k2]))
floop.close()
