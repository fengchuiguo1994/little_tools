...
interactions <- read.table("diffloop.DESeq2.final.cancer.bedpe")
...
cyto <- read.table("~/cytoBand.txt", fill = T, header = F)
cyto <- cyto[nchar(as.character(cyto[[1]])) <= 5, ]
layout_nrow <- length(unique(cyto[[1]]))
peaks <- read.table("/data/home/ruanlab/huangxingyu/CSCC/DNase/handle/align/test.peaks")
peaks[[4]] <- (peaks[[4]] - min(peaks[[4]]))/(max(peaks[[4]]) - min(peaks[[4]]))
pdf("test.curve.pdf", width = 10, height=20)
...
interaction$PlotColor <- apply(interaction, 1, function(x){x[7] <- as.numeric(x[7])
  if(x[7] == 2){a = "#EF667A"}
...
for (i in seq.int(length(chrom_no))) { 
  ...
  if (nrow(chrom_centro) > 0) {
    ...
    # chrom_left$start <- 0
    # grid.rect(x = unit(chrom_left[["start"]], "native"), y = unit(1, "npc"), width = unit(chrom_left[["end"]] - chrom_left[["start"]], "native"), height = unit(1, "npc"), just = c("left", "top"), gp = gpar(fill = NA, col = "red"), vp = vp)   
    xx <- c(unit(chrom_left[["start"]], "native"), unit(chrom_left[["start"]], "native"), unit(chrom_left[["end"]], "native"))  # x坐标
    yy <- c(unit(0, "npc"), unit(1, "npc"), unit(0.5, "npc"))  # y坐标
    grid.polygon(xx, yy, gp = gpar(fill = "red", col = "red"), vp = vp, default.units = "native") # 绘制三角形
    
    # Right part
    chrom_right <- chrom_centro[2, ]
    # chrom_right$end <- chrom_max
    # grid.rect(x = unit(chrom_right[["start"]], "native"), y = unit(1, "npc"), width = unit(chrom_right[["end"]] - chrom_right[["start"]], "native"), height = unit(1, "npc"), just = c("left", "top"), gp = gpar(fill = NA, col = "red"), vp = vp)
    xx <- c(unit(chrom_right[["start"]], "native"), unit(chrom_right[["end"]], "native"), unit(chrom_right[["end"]], "native"))  # x坐标
    yy <- c(unit(0.5, "npc"), unit(0, "npc"), unit(1, "npc"))  # y坐标
    grid.polygon(xx, yy, gp = gpar(fill = "red", col = "red"), vp = vp, default.units = "native") # 绘制三角形
  }
